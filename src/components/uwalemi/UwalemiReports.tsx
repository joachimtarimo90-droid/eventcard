...
export const UwalemiReports: React.FC<Props> = ({ state, onOpenSmsWithTemplate }) => {
  ...
  const [reportType, setReportType] = useState<'financial' | 'members' | 'emergency'>('financial');
  ...
  const handleExportExcel = () => {
    if (reportType === 'financial') {
      ...
      const rows = members.map(m => {
        ...
        const memberTotalDebt = regDebt + feeDebt + finesDebt;
        ...
        rowData['Faini Zilizolipwa'] = finesPaid;
        rowData['Michango ya Dharura'] = emergencyPaid;
        rowData['Jumla ya Fedha Alizotoa'] = memberTotalContributed;
        rowData['Jumla ya Madeni'] = memberTotalDebt;
        rowData['Hali ya Malipo'] = memberTotalDebt === 0 ? 'Amelipa' : paidFee > 0 ? 'Pungufu' : 'Ana Deni';
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-12" id="uwalemi-reports-center">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-slate-900/60 p-5 rounded-2xl border border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Tengeneza na pakua ripoti safi za PDF zenye barua rasmi ya UWALEMI, mihuri, saini na takwimu kamili za fedha.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            ...
            {isGenerating ? 'Inatengeneza PDF...' : 'Pakua Ripoti ya PDF'}
          </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            ...
            >
              ...
              <div>
                ...
                <span className="text-[10px] text-slate-400">Mapato, matumizi na salio la hazina</span>
              </div>
            </button>
            ...
            >
              ...
              <div>
                ...
                <span className="text-[10px] text-slate-400">Orodha, ada zilizolipwa na madeni</span>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <span className="text-xs font-bold block text-white">Michango ya Dharura & Misiba</span>
                <span className="text-[10px] text-slate-400">Waliochanga na wasiochanga kwa kila dharura</span>
              </div>
            </button>
          </div>
        </div>
      ...
      <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-4 sm:p-8 backdrop-blur-md" id="printable-report-area">
                ...
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-800/60">
                    {members.map((m, idx) => {
                      ...
                      const totDebt = regDebt + feeDebt + finesDebt;
                      ...
                      return (
                        <tr key={m.id} className="hover:bg-slate-900/50 transition-colors">
                          ...
                          <td className="p-3 text-right font-mono text-slate-300">{formatTZS(finesPaid)}</td>
                          ...
                          <td className="p-3 text-center">
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${totDebt === 0 ? 'bg-emerald-500/20 text-emerald-400' : paidFee > 0 ? 'bg-amber-500/20 text-amber-400' : 'bg-rose-500/20 text-rose-400'}`}>
                              {totDebt === 0 ? 'AMELIPA' : paidFee > 0 ? 'PUNGUFU' : 'ANA DENI'}
                            </span>
                          </td>
                        </tr>
                      );
                        ...
                        <tr key={m.id || idx} className="hover:bg-slate-900/40">
                          ...
                          <td className="p-3 text-center">
                            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold ${
                              memberTotalDebt === 0 ? 'bg-emerald-500/20 text-emerald-300' : paidFee > 0 ? 'bg-amber-500/20 text-amber-300' : 'bg-rose-500/20 text-rose-400'
                            }`}>
                              {memberTotalDebt === 0 ? 'Amelipa' : paidFee > 0 ? 'Pungufu' : 'Ana Deni'}
                            </span>
                          </td>
                        </tr>
                      );
        ...
        {/* 3. EMERGENCY FUND REPORT PREVIEW */}
